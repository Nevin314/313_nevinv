...
int main (int argc, char *argv[]) {
    ...
    if (pid == -1) {
        ...
        char* args[] = {(char*) "./server",(char*) "-m",  (char*) buf_size.data(), NULL};
    ...
    FIFORequestChannel chan_default("control", FIFORequestChannel::CLIENT_SIDE); 
    channels.push_back(&chan_default);
    FIFORequestChannel* chan = channels.back();

    if (newch) {
        MESSAGE_TYPE nc = NEWCHANNEL_MSG; 
        chan_default.cwrite(&nc, sizeof(MESSAGE_TYPE)); 
    
        char name_buf[30]; 
        chan_default.cread(name_buf, 30);
